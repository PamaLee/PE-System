...
if (isset($_POST['submit'])){
    ...
    foreach ($student as $row){
        ...
        if (!empty($_POST[$uid."-choose"])){
            ...
            $choosees=$_POST[$uid.'-choosen'];
            $er=link_admin()->query("update test_res set choose_what='$choosees' where school='$school' and test_num='$test_num' and grade='$grade' and class='$class' and uid='$uid'");
            if (!$er){
                echo $_POST[$uid.'-choosen'];
            }else{
                echo $_POST[$uid.'-choosen'];
...
<form action="app/xl_s.php" method="post">
    <?php
    $school=$_SESSION['info']['school'];
    $grade=$_SESSION['info']['grade'];
    $class=$_GET['class'];
...
<div class=\"mdui-row-xs-5\">
    ...
    <div class=\"mdui-col\">
        ...
        echo "<select class='mdui-select' name='$uid-choosen' id='$uid-choosen' mdui-select='{position: bottom}'>";
        echo "<option value=\"1\" ";
        if ($test_choose==1){echo "selected";}
        echo ">实心球</option>";
        echo "<option value=\"2\" ";
        if ($test_choose==2){echo "selected";}
        echo ">立定跳远</option>";
        echo "<option value=\"3\" ";
        if ($test_choose==5){echo "selected";}
        echo ">引体向上</option>";
        echo "<option value=\"4\" ";
        if ($test_choose==4){echo "selected";}
        echo ">跳绳</option>";
        echo "</select>";
            echo "</h3>
    </div>
...
echo "<select class='mdui-select' name='$uid-choosen' id='$uid-choosen' mdui-select='{position: bottom}'>";
                    ...
                    echo ">立定跳远</option>";
                    echo "<option value=\"3\" ";
                    if ($test_choose==3){echo "selected";}
                    echo ">仰卧起坐</option>";
                    echo "<option value=\"4\" ";
                    if ($test_choose==4){echo "selected";}
                    echo ">跳绳</option>";
