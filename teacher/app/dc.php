...
if (isset($_GET['c'],$_GET['test']) and $_GET['c']=="daochu")
{
...
$sql = "SELECT * FROM test_res where school='$school' and grade='$grade' and class='$class'";
...
if ($result->num_rows > 0) {
    while($row = mysqli_fetch_array($result)) {
        $uid=$row['uid'];
        $students=link_admin()->query("select * from student where uid='$uid'")->fetch_array();
        $choose_name=get_choose_name($students['choose_what']);
        echo $students['study_hao']."\t".$students['name']."\t".$row['short_run_res']."\t".$row['long_run_res']."\t".$row['choose_res']."\t".$choose_name."\t".$row['short_run_sc']."\t".$row['long_run_sc']."\t".$row['zong_sc']."\t\n";
    }
}else{
    echo "没有查询数据!";
}
